...
<div class="max-w-4xl mx-auto px-4 py-6 space-y-8">
    ...
    <div class="grid gap-6 md:grid-cols-2">
        ...
        {{-- Carica libro --}}
        <section class="rounded-2xl bg-white border border-slate-200 p-5">
            <h2 class="text-xl font-bold mb-3">📤 Carica un libro (PDF)</h2>
            @if ($subjects->isEmpty())
                <p class="text-slate-500">Prima aggiungi almeno una materia.</p>
            @else
                <form action="{{ route('books.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                    ...
                    <select name="subject_id" required
                        ...
                        <option value="">— Materia del libro —</option>
                        @foreach ($subjects as $subject)
                            <option value="{{ $subject->id }}">{{ $subject->name }}</option>
                        @endforeach
                    </select>
                    <input type="text" name="title" maxlength="150" placeholder="Titolo (facoltativo)"
                           class="w-full rounded-xl border-2 border-slate-300 px-4 py-2 text-lg focus:border-sky-500 focus:outline-none">
    ...
    {{-- Elenco libri --}}
    <section>
        <h2 class="text-xl font-bold mb-3">📚 Libri caricati</h2>
        @php
            $badge = [
        ...
        @forelse ($books as $book)
            <div class="flex items-center gap-4 rounded-2xl bg-white border border-slate-200 p-4 mb-3"
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-sm text-slate-500">{{ $book->subject->name ?? '—' }}</p>
                ...
                <span class="js-status rounded-full px-3 py-1 text-sm font-semibold {{ $badge[$book->status] ?? 'bg-slate-200' }}">
                    {{ $book->statusLabel() }}@if($book->status === 'ready') · {{ $book->chunk_count }} parti @endif
                </span>
                <form action="{{ route('books.destroy', $book) }}" method="POST"
                      onsubmit="return confirm('Vuoi eliminare questo libro?')">
                    @csrf @method('DELETE')
                    <button class="rounded-xl px-3 py-1 text-slate-400 hover:text-red-600" title="Elimina">🗑️</button>
                </form>
            </div>
        @empty
            <p class="text-slate-500">Nessun libro ancora. Carica il primo PDF qui sopra.</p>
        @endforelse
    </section>
</div>

<script>
    // Polling leggero dello stato dei libri ancora in lavorazione.
    document.querySelectorAll('[data-book-id]').forEach((row) => {
        const badge = row.querySelector('.js-status');
        const text = badge.textContent.trim().toLowerCase();
        if (!text.startsWith('in coda') && !text.startsWith('in lavorazione')) {
            return;
        }
