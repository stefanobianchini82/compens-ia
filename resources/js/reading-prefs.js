...
const buildPanel = (prefs, onChange) => {
    ...
    panel.setAttribute('aria-label', t('reading_prefs', 'Preferenze di lettura'));
    panel.className =
    ...
    const fontOptions = [
        ['default', t('reading_font_default', 'Predefinito')],
    ...
    panel.innerHTML = `
        <h2 class="text-xl font-bold mb-4">${t('reading_heading', '🔤 Come leggi meglio?')}</h2>
        <label class="block mb-4">
            <span class="font-semibold">${t('reading_font', 'Carattere')}</span>
        ...
        <label class="block mb-4">
            <span class="font-semibold">${t('reading_size', 'Dimensione testo')}</span>
            <input id="rp-scale" type="range" min="0.9" max="1.6" step="0.05" value="${prefs.scale}" class="mt-1 w-full">
        </label>
        <label class="block mb-4">
            <span class="font-semibold">${t('reading_line', 'Spazio tra le righe')}</span>
            <input id="rp-line" type="range" min="1.4" max="2.4" step="0.1" value="${prefs.line}" class="mt-1 w-full">
        </label>
        <label class="block mb-4">
            <span class="font-semibold">${t('reading_letter', 'Spazio tra le lettere')}</span>
        ...
        <label class="flex items-center gap-3 mb-5">
            ...
            <span class="font-semibold">${t('reading_ruler', 'Righello di lettura')}</span>
        </label>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!toggle) {
        ...
        toggle.setAttribute('aria-label', t('reading_prefs', 'Preferenze di lettura'));
        toggle.className =
